...
class PDFReportGenerator:
    ...
    def generate_report(self, conversation_data, session_id):
        ...
        # Add detailed recommendations with commands
        story.append(Paragraph("Detailed Recommendations & Commands", self.styles['SectionHeader']))
        detailed_recs = self.get_detailed_recommendations_with_commands(conversation_data.get('type'), summary_data)
        story.append(Paragraph(detailed_recs, self.styles['Normal']))
        story.append(Spacer(1, 20))
        ...
        # Add performance metrics table for all service types
        ...
        # Add problem areas analysis
        story.append(Paragraph("Problem Areas & Solutions", self.styles['SectionHeader']))
        problem_analysis = self.create_problem_areas_table(conversation_data.get('type'))
        if problem_analysis:
            story.append(problem_analysis)
            story.append(Spacer(1, 15))
    ...
    def get_detailed_recommendations_with_commands(self, service_type, summary_data):
        """Get detailed recommendations with specific commands"""
        if service_type == 'query':
            return """<b>SQL Query Optimization Commands:</b><br/><br/>
            <b>1. Analyze Current Performance:</b><br/>
            <font name="Courier" size="8">EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT) SELECT * FROM your_table WHERE conditions;</font><br/><br/>
            <b>2. Create Recommended Indexes:</b><br/>
            <font name="Courier" size="8">CREATE INDEX CONCURRENTLY idx_table_column ON table_name(column_name);<br/>
            CREATE INDEX CONCURRENTLY idx_composite ON table_name(col1, col2, col3);</font><br/><br/>
            <b>3. Update Table Statistics:</b><br/>
            <font name="Courier" size="8">ANALYZE table_name;<br/>
            VACUUM ANALYZE table_name;</font><br/><br/>
            <b>4. Monitor Query Performance:</b><br/>
            <font name="Courier" size="8">SELECT query, calls, total_time, mean_time FROM pg_stat_statements ORDER BY total_time DESC LIMIT 10;</font>
            """
        elif service_type == 'performance':
            return """<b>Performance Tuning Commands:</b><br/><br/>
            <b>1. Check Current Resource Usage:</b><br/>
            <font name="Courier" size="8">SELECT * FROM pg_stat_activity WHERE state = 'active';<br/>
            SELECT schemaname, tablename, n_tup_ins, n_tup_upd, n_tup_del FROM pg_stat_user_tables;</font><br/><br/>
            <b>2. Identify Blocking Queries:</b><br/>
            <font name="Courier" size="8">SELECT blocked_locks.pid AS blocked_pid, blocking_locks.pid AS blocking_pid FROM pg_catalog.pg_locks blocked_locks;</font><br/><br/>
            <b>3. Configure Performance Parameters:</b><br/>
            <font name="Courier" size="8">ALTER SYSTEM SET shared_buffers = '256MB';<br/>
            ALTER SYSTEM SET work_mem = '4MB';<br/>
            SELECT pg_reload_conf();</font>
            """
        else:
            return """<b>General Database Commands:</b><br/><br/>
            <b>1. Health Check:</b><br/>
            <font name="Courier" size="8">SELECT datname, numbackends, xact_commit, xact_rollback FROM pg_stat_database;</font><br/><br/>
            <b>2. Performance Monitoring:</b><br/>
            <font name="Courier" size="8">SELECT schemaname, tablename, seq_scan, seq_tup_read, idx_scan, idx_tup_fetch FROM pg_stat_user_tables;</font>
            """
    
    def create_problem_areas_table(self, service_type):
        """Create problem areas analysis table"""
        if service_type == 'query':
            data = [
                ['Problem Area', 'Severity', 'Root Cause', 'Solution'],
                ['Missing Indexes', 'High', 'No indexes on WHERE clauses', 'CREATE INDEX statements'],
                ['Full Table Scans', 'High', 'Inefficient query patterns', 'Query rewriting + indexes'],
                ['Outdated Statistics', 'Medium', 'Stale table statistics', 'ANALYZE commands'],
                ['Poor Query Design', 'Medium', 'Suboptimal SQL structure', 'Query optimization']
            ]
        elif service_type == 'performance':
            data = [
                ['Problem Area', 'Severity', 'Root Cause', 'Solution'],
                ['High CPU Usage', 'High', 'Inefficient queries', 'Query optimization'],
                ['Memory Pressure', 'High', 'Poor buffer management', 'Memory tuning'],
                ['I/O Bottlenecks', 'Medium', 'Disk performance issues', 'Storage optimization'],
                ['Connection Limits', 'Medium', 'Pool exhaustion', 'Connection pooling']
            ]
        else:
            data = [
                ['Problem Area', 'Severity', 'Root Cause', 'Solution'],
                ['Configuration Issues', 'Medium', 'Suboptimal settings', 'Parameter tuning'],
                ['Monitoring Gaps', 'Low', 'Limited visibility', 'Enhanced monitoring'],
                ['Maintenance Tasks', 'Low', 'Irregular maintenance', 'Automated scheduling'],
                ['Documentation', 'Low', 'Outdated procedures', 'Update documentation']
            ]
        
        table = Table(data, colWidths=[1.8*inch, 1*inch, 2*inch, 1.7*inch])
        table.setStyle(TableStyle([
